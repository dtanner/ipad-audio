...
- `iPadAudio/Models/AudioConstants.swift` — SAMPLE_RATE (48000), BLOCK_SIZE (4800), SPL_MIN (20), SPL_MAX (100), CALIBRATION_DB (94), threshold defaults
...
- `iPadAudio/Models/PanelType.swift` — `enum PanelType: String, CaseIterable { case meter, pitch }`
...
- `iPadAudio/Views/SettingsView.swift` — SwiftUI Form/NavigationStack: sliders for history length, thresholds, pitch note range, pitch auto toggle. Presented as `.sheet` from gear icon, not full-screen navigation
...
## Phase 3: YIN Pitch Detection + Tuner + Pitch Chart
**Status:** COMPLETE
...
## Phase 4: Panel Layout + Toggle Buttons
**Status:** COMPLETE
...
**How to test:** Toggle panel combinations. Verify layout adapts. Panel state persists across launches. Test in Split View — should force single panel.
